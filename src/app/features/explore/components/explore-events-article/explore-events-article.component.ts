...
export class ExploreEventsArticleComponent implements OnInit {
  ...
  prettyUrl(childRoute: string) {
    childRoute = childRoute.replace(/%E2%80%99/g, "’").replace(/%20/g, ' ').replace(/%E2%80%93/g, '–').replace(/%C3%B1/g, 'ñ')
    return childRoute
  }
